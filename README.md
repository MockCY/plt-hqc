...
训练计划按天从动作库编排需执行 `database/40-plan-day-exercises.sql`。脚本会新增计划日、计划日动作和用户完成日记录，并为训练计划增加详情图片字段；现有基于课程的计划条目会按课程动作自动迁移。应用不会自动执行此迁移。

训练计划支持独立周期天数需执行 `database/41-plan-cycle-days.sql`。周期可以多于训练日数量，但不能少于训练日数量；迁移会用现有训练日数量初始化历史计划。应用不会自动执行此迁移，必须先迁移数据库再启动新版服务。

训练计划首页展示图需执行 `database/42-plan-home-image.sql`。后台可为首页“当前训练计划”卡片单独上传横图；未配置时小程序继续使用列表展示图。迁移只新增可空字段，不复制或覆盖已有图片。

多计划并行需执行 `database/43-multiple-user-plans.sql`。一个用户可以同时选择多个计划；首页优先展示最近查看的启用计划，没有查看记录时展示最近选择的启用计划，不再自动展示排序第一的计划。迁移会保留已有选择和选择时间，应用不会自动执行此迁移。
