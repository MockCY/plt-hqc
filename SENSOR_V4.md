...
V4 `durationMs` 采用 `activeDurationMs`，暂停不增加有效时长。静止 5 分钟完成会话，随后到达的摘要仍更新原行；V3 保留原有 3 分钟及首末运动跨度口径。V3/V4 已完成记录仅在往返次数大于 0 时出现在训练历史和统计；0 次记录无论时长多少，均不计入已完成训练数、总时长和训练天数。进行中记录在首次计次前仍保留展示，但尚不贡献统计时长和天数。该规则在查询时应用于已有记录，原始上报和会话保留，无需删除数据或执行迁移；迟到的 V4 摘要补齐正次数后，同一记录会重新纳入历史和统计。未知或仅估算历法时间的有效训练可计入已确认归属账号的总时长/次数，但不计算训练日期或连续天数。
